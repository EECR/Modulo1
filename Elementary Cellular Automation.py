...
def cellular_automaton(pattern, value, gen):
    new_pattern = ''
    i = -1
    active_positions = []
    list_values = [1, 2, 4, 8, 16, 32, 64, 128]
    key = {'...':'.', '..x':'.', '.x.':'.', '.xx':'.', 'x..':'.', 'x.x':'.', 'xx.':'.', 'xxx':'.'}
    for e in list_values[::-1]:
        if value / e > 0:
            pos = list_values.index(e)
    ...
    while x != gen:
        #New Pattern
        new_pattern += key[pattern[-1] + pattern[0:2]]
        for i in range((len(pattern) - 2)):
            new_pattern += key[pattern[i:i+3]]
            new_pattern += key[pattern[-2:] + pattern[0]]
        pattern = new_pattern
        new_pattern = ''
        x += 1
...
cellular_automaton('.x.x.x.x.', 17, 2)    
